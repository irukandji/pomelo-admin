...
var clientLogger = require('pomelo-logger').getLogger('admin-log', __filename);
  ...
  this.server.on('connection', function(socket) {
    ...
    socket.on('disconnect', function() {
      ...
      if (type === 'client' && registered) {
          clientLogger.info('client user:"%s" with id:"%s" exit', username, id);
      }
      registered = false;
      id = null;
      type = null;
...
var ipFilter = function(obj) {
  if(typeof this.whitelist === 'function') {
    ...
    self.whitelist(function(err, tmpList) {
      if(err) {
          clientLogger.error('%j.(pomelo-admin whitelist).', err);
        return;
      }
      if(!Array.isArray(tmpList)) {
          clientLogger.error('%j is not an array.(pomelo-admin whitelist).', tmpList);
        return;
      }
      if(!!obj && !!obj.ip && !!obj.id) {
        ...
        if(sock) {
          sock.disconnect('unauthorized');
            clientLogger.error('%s is rejected(pomelo-admin whitelist).', obj.ip);
        }
      }
    });
  }
...
var doAuthUser = function(msg, socket, self, cb) {
  ...
  authUser(msg, env, function(user) {
    ...
    if (self.clients[msg.id]) {
      ...
      clientLogger.info('client user:"%s" with id:"%s" login to master', username, msg.id);
